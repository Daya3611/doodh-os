...
export default function AddFarmerPage() {
  ...
  const onSubmit = async (data: FarmerFormData) => {
    ...
    setIsLoading(true);
    try {
      const farmerId = await farmerService.add(centerId, data);
      toast.success(`Farmer added with ID: ${farmerId}`);
      router.push('/farmers');
    } catch (error: any) {
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-[16px] font-bold text-[#111111]">Add New Farmer</div>
        </div>
      </div>
      ...
      <div
        ...
        style={{ background: '#FFF3E8', border: '1px solid #FFD4AB' }}
      >
        <Info size={16} style={{ color: '#FF6B00', flexShrink: 0 }} />
        <span className="text-[13px] text-[#CC5500]">
          Farmer ID will be automatically generated in the format <strong>F001, F002…</strong>
        </span>
      </div>
