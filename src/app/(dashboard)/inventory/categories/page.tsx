...
export default function CategoriesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!centerId || !newCategory.trim()) return;
    ...
    try {
      await inventoryService.saveInventorySettings(centerId, { categories: updated });
  ...
  const handleDelete = async (catToDelete: string) => {
    if (!centerId) return;
    if (!confirm(`Are you sure you want to delete "${catToDelete}"?`)) return;
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex justify-between items-center">
        <h2 className="text-[18px] font-extrabold text-[#111111] uppercase tracking-wide">Manage Categories</h2>
      </div>
      ...
      <div style={cardStyle} className="p-6 space-y-4">
        ...
        <form onSubmit={handleAdd} className="flex gap-3">
          <input
            type="text"
            required
            placeholder="e.g. Dairy Wash, Feeds, Vaccines..."
            value={newCategory}
              ...
              {categories.map((cat, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-[#111111]">{cat}</span>
                  </div>
                  <button
